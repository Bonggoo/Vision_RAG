# 📑 대용량 업로드 고도화 — 비동기 GCS 다이렉트 업로드 아키텍처 (구현 완료)

대용량 매뉴얼(32MB+) 업로드 시 Cloud Run 메모리 OOM 이슈를 근본적으로 해결하고,
업로드 속도와 UX를 극대화하기 위해 개발 완료 및 반영된 **"Pre-flight + GCS Direct + Async Worker"** 아키텍처 보고서입니다.

---

## 1. 현행 업로드 방식의 병목 분석 (개선 전)
...
*   **현재 상태**: Pre-flight 사전 검증, GCS Direct 업로드(XMLHttpRequest), BackgroundTasks 비동기 분석 파이프라인으로 구성된 하이브리드 업로드 아키텍처가 전면 적용되어 정상 작동 중입니다.
*   **해결된 이점**: 20MB 이상 대용량 파일 업로드 시 Cloud Run 서버 메모리 소모가 0MB화되었으며, 사용자는 업로드 즉시 로딩 없이 사이드바에서 분석 상태를 실시간 확인(analyzing 배지)하며 서비스를 지속 이용할 수 있습니다.
*   **보강된 검증**: GCS CORS 설정 변경(PUT 메서드 허용) 및 서비스 계정 권한 정밀 조정을 통해 프로덕션 환경의 안정성을 확보했습니다.
