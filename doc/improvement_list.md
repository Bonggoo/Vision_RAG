...
* **[x] 로그인 과정 자연스럽게**
  ...
  * **세션 갱신 회복력**: 리프레시 서버가 일시 오류(5xx/네트워크)일 땐 기존 세션을 유지하고 실제 만료(401)에서만 강제 로그아웃하도록 개선, 문서 재분석 요청 누락 인증 헤더 보강 (PR #12·#13)
* **[x] 구글 OAuth 계정 정리 및 연동**
  * 프로덕션 서버와 로컬 환경 간 구글 API 및 JWT 인증 키 설정 통합 및 환경변수 최신화
* **[x] 접근 제어 및 사용자 격리**
  * ~~화이트리스트 기반 접근 제어~~ → **폐지** (커밋 `6321e4d`): 구글 계정으로 정상 로그인한 사용자라면 누구나 접근 가능한 공개 서비스로 전환
...
* **[x] 인앱 알림 체계 & 온보딩 고도화** (PR #10·#11)
  * 네이티브 `alert()/confirm()`를 전역 `useUIStore` 기반 토스트(`toast.*`)와 확인 다이얼로그(`confirmDialog()`)로 대체 — 컴포넌트 밖에서도 호출 가능, 파괴적 동작은 위험 스타일. 업로드 결과(성공/중복/실패)를 `lib/upload.ts` 공용 핸들러에서 요약 안내
  * 보유 문서가 없는 신규 사용자에게 적응형 웰컴 온보딩 화면 노출, 문서 로딩 중 온보딩↔질문 화면 플래시 제거
