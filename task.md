...
- [/] 2차 UX 피드백 반영 및 에이전트 라우팅 고도화
  - [/] 업로드 완료 시 사이드바가 닫히지 않도록 수정 (`Sidebar.tsx` handleFileUpload 수정)
  - [/] 라이트 모드일 때 새 대화 시작 버튼 글자 색상 수정 (`Sidebar.tsx` btn-primary 텍스트 오버라이드)
  - [/] 모바일에서 문서 수정/삭제 아이콘 항시 노출 (`Sidebar.tsx` opacity 반응형 클래스 변경)
  - [/] 모바일에서는 사이드바가 열렸을 때만 15초 주기 조건부 폴링 기능 구현 (`Sidebar.tsx` useEffect 최적화)
  - [/] 백엔드 단순 일상 대화("안녕" 등) 분류 및 가이드 대화 라우팅 추가 (`agentic_graph.py` 수정)
- [ ] 검증 및 빌드 확인
  - [ ] 로컬 제목 추출 및 안녕 라우팅 테스트 진행
  - [ ] 프론트엔드 빌드 검증 (`npm run build`)
  - [ ] Git 원격 브랜치 커밋 및 PR 생성
