"""
app/services/agentic/ 순수 함수 단위 테스트.
...
# ─── 테스트 대상 import ─────────────────────────────────────────────────────
from app.services.agentic.classification import quick_classify
...
from app.services.agentic.toc import (
...
class TestNormalizePage:
    def test_int_passthrough(self):
        assert normalize_page(42) == 42

    def test_float_truncates(self):
        assert normalize_page(3.9) == 3

    def test_string_integer(self):
        assert normalize_page("32") == 32

    def test_chapter_page_format(self):
        # "3-32" → 32 (하이픈 뒤 숫자)
        assert normalize_page("3-32") == 32

    def test_chapter_page_single_digit(self):
        assert normalize_page("1-1") == 1
...
class TestExtractModelCodes:
    ...
    def test_extracts_alphanumeric_model(self):
        ...
        assert extract_model_codes("F388A를 생산 라인에 설치") == {"f388a"}

    def test_extracts_hyphenated_model(self):
        assert extract_model_codes("LS-R900의 고속 응답 모드") == {"ls-r900"}
        assert extract_model_codes("CZ-V20 컬러 센서") == {"cz-v20"}

    def test_underscore_splits_token(self):
        # SV_MEMORY는 밑줄로 끊겨 제외되고, 실제 모델(L7NH)만 남음
        assert extract_model_codes("SV_MEMORY 알람 L7NH 서보") == {"l7nh"}

    def test_alarm_code_excluded(self):
        # 'AL.20' — 점으로 끊겨 'AL'(숫자 없음)·'20'(문자 없음) 둘 다 탈락
        assert extract_model_codes("AL.20 알람이 떠 있는데") == set()

    def test_two_char_token_excluded(self):
        # '2D' — 2자 이하는 제외 (오탐 방지)
        assert extract_model_codes("GS1 2D 심볼과 선형 바코드") == {"gs1"}

    def test_pure_korean_returns_empty(self):
        assert extract_model_codes("종단 저항은 별도로 부착해야 하나요") == set()

    def test_model_disambiguation_f388a_vs_f381(self):
        # 핵심 회귀 방어: 'F388A' 질문이 'F381' 문서로 새면 안 됨.
        q = extract_model_codes("F388A의 최대 하중과 샘플링 속도는?")
        doc_388 = extract_model_codes("유니펄스 F388A 웨이브 폼 체커 사용 설명서")
        doc_381 = extract_model_codes("유니펄스 F381 취급설명서")
...
class TestStripJosa:
    def test_strips_common_particles(self):
        assert strip_josa("색상의") == "색상"
        assert strip_josa("설정값을") == "설정값"
        assert strip_josa("센서가") == "센서"
        assert strip_josa("라인에서") == "라인"

    def test_strips_longest_particle_first(self):
        # '차이만으로' → '차이만' + '으로'가 아니라 '차이' + '만으로'
        assert strip_josa("차이만으로") == "차이"

    def test_protects_short_roots(self):
        # 어근이 2자 미만으로 남으면 떼지 않음 ('차이'의 '이'는 조사가 아니라 어근)
        assert strip_josa("차이") == "차이"
        assert strip_josa("회로") == "회로"
        assert strip_josa("온도") == "온도"

    def test_ascii_untouched(self):
        assert strip_josa("manual") == "manual"
        assert strip_josa("f388a") == "f388a"
...
class TestFilterDocuments:
    ...
    def test_josa_stripped_keyword_matches(self):
        ...
        docs, mode, _ = filter_documents_by_keywords("서보의 알람 원인", self._corpus())
    ...
    def test_df_cut_ignores_corpus_generic_words(self):
        ...
        # → 남는 변별 키워드가 없어 전체 폴백
        docs, mode, _ = filter_documents_by_keywords("설치 방법 알려줘", self._corpus())
    ...
    def test_weak_evidence_falls_back_to_all(self):
        ...
        docs, mode, _ = filter_documents_by_keywords("버퍼 메모리 읽기", self._corpus())
        if mode == "filtered":
    ...
    def test_model_code_match_is_trusted(self):
        docs, mode, _ = filter_documents_by_keywords("L7NH 알람 확인", self._corpus())
        assert mode == "filtered"
        assert docs[0]["document_id"] == "servo"

    def test_no_match_returns_all(self):
        docs, mode, _ = filter_documents_by_keywords("김치찌개 끓이는 법", self._corpus())
...
class TestTocEvidence:
    ...
    def test_toc_only_term_collected_as_evidence(self):
        ...
        docs, mode, ev = filter_documents_by_keywords("SMATV 광방송장비 라인업", self._corpus())
        assert "neo" in ev
    ...
    def test_generic_words_produce_no_evidence(self):
        ...
        _, _, ev = filter_documents_by_keywords("설치 방법 알려줘", self._corpus())
        assert ev == {}

    def test_no_match_no_evidence(self):
        _, _, ev = filter_documents_by_keywords("김치찌개 끓이는 법", self._corpus())
        assert ev == {}
    ...
    def test_evidence_capped_per_doc(self):
        ...
        _, _, ev = filter_documents_by_keywords("위빙모드 설정", [big] + self._corpus())
...
class TestSseEvent:
    def test_event_format(self):
        result = sse_event("answer", text="안녕하세요")
        assert result.startswith("data: ")
        assert result.endswith("\n\n")

    def test_event_contains_type(self):
        result = sse_event("reasoning", text="탐색 중")
    ...
    def test_extra_kwargs_included(self):
        result = sse_event("reference", page=5, image="base64data")
        payload = json.loads(result.removeprefix("data: ").strip())
...
class TestQuickClassify:
    def test_greeting_short_korean(self):
        assert quick_classify("안녕하세요") == "general"

    def test_greeting_english(self):
        assert quick_classify("hello") == "general"

    def test_technical_error_keyword(self):
        assert quick_classify("AL.10 에러가 발생했습니다") == "technical"

    def test_technical_alarm_keyword(self):
        assert quick_classify("알람 코드 확인 방법") == "technical"

    def test_technical_parameter(self):
        assert quick_classify("파라미터 설정값을 바꾸려면") == "technical"

    def test_ambiguous_returns_none(self):
        # LLM 판별이 필요한 모호한 질문
        assert quick_classify("이 장비 사용 방법을 알려주세요") is None

    def test_long_greeting_is_not_general(self):
        # 20자 초과 → greeting 패턴 적용 안 됨
        assert quick_classify("안녕하세요 저는 오늘 이 장비에 대해 여쭤보고 싶습니다") is None
...
class TestGenerateDefaultClarificationQuestions:
    ...
    def test_returns_at_most_3_questions(self):
        questions = build_default_clarification_questions("알람 확인", self._docs())
    ...
    def test_ignores_misang_values(self):
        questions = build_default_clarification_questions("알람", self._docs())
    ...
    def test_dedupes_same_prefix(self):
        ...
        questions = build_default_clarification_questions("알람", docs)
    ...
    def test_skips_parts_already_in_question(self):
        ...
        questions = build_default_clarification_questions(question, docs)
    ...
    def test_returns_empty_when_nothing_to_add(self):
        ...
        questions = build_default_clarification_questions(
            "미쯔비시 Q 시리즈 통신 에러", docs)
        assert questions == []

    def test_empty_docs(self):
        # 재작성할 근거가 없으면 빈 리스트 (프론트에서 추천 질문 섹션 숨김)
        questions = build_default_clarification_questions("알람", [])
        assert questions == []
...
class TestResolveTargetPagesWithoutToc:
    ...
    def test_small_doc_returns_all_pages(self):
        assert resolve_target_pages_without_toc(1) == [1]
        assert resolve_target_pages_without_toc(3) == [1, 2, 3]

    def test_at_limit_returns_all_pages(self):
        pages = resolve_target_pages_without_toc(SMALL_DOC_FULL_SCAN_PAGES)
        assert pages == list(range(1, SMALL_DOC_FULL_SCAN_PAGES + 1))

    def test_large_doc_returns_none(self):
        assert resolve_target_pages_without_toc(SMALL_DOC_FULL_SCAN_PAGES + 1) is None
        assert resolve_target_pages_without_toc(300) is None
