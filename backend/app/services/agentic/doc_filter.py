...
# 목차에 실제로 등장하지만 주제를 담지 않는 일반어 — '증거 쪽지' 대상에서만 제외한다.
#
# DF 컷은 '코퍼스에서 희귀한가'만 보므로 이들을 걸러내지 못한다. 예를 들어 '처음'은
# 55개 문서 중 4개 목차에만 나와 희귀 컷을 통과하지만("처음에", "처음으로 전원을
# 투입하는 경우"), 질문의 주제와는 무관하다. 그 결과 "이거 처음 써보는데 뭐부터 보면
# 될까요?" 같은 일상 질문에도 "★ 질문 키워드와 일치하는 목차 항목" 블록이 만들어져,
# 문서 선택 LLM 이 이를 매뉴얼 관련 질문(technical)의 근거로 읽었다.
#
# ⚠️ 점수 계산에는 적용하지 않는다. 증거에만 적용하면 실수의 양방향이 모두 안전하다 —
# 빠뜨린 단어는 잡음 쪽지 하나(현상 유지), 과하게 넣은 단어는 쪽지 하나를 잃을 뿐
# 문서가 후보에서 탈락하지는 않는다. 이 파일의 'recall 우선' 원칙과도 맞는다.
#
# ⚠️ 산업 매뉴얼에서 주제어로 쓰이는 말은 넣지 말 것. 특히 '이상'은 넣고 싶어지지만
# "링크 이상국", "이상 발생 시"처럼 고장을 뜻하는 핵심어다. '시작'(운전 시작),
# '정보'(윤곽 정보), '구성'(시스템 구성)도 같은 이유로 제외했다.
# 아래 목록은 실제 55개 문서 목차에 대조해 비주제성을 확인한 것이다(2026-08-15).
_NON_TOPICAL_WORDS = {
    # 지시어·대명사
    "이거", "그거", "저거", "이것", "그것", "저것", "여기", "거기", "이때", "이번", "자체",
    # 시점·순서
    "처음", "다음", "이전", "마지막", "나중",
    # 수량·범위
    "각각", "모두", "전부", "일부",
    # 의존명사·연결어
    "경우", "경우에", "때문", "대해", "대한", "관해", "관련", "위해", "위한", "정도", "이하",
    # 기타 일반어
    "참고", "기타", "사항",
}
...
def _select_discriminative_keywords(
    ...
    - 비주제어 컷: 빈도로는 갈리지 않는 '처음'·'다음' 류를 증거에서만 제외
      (_NON_TOPICAL_WORDS 참고). 희귀도와 의미는 별개라 컷 값 조정으로는 해결되지 않는다.
    """
    ...
    for kw in keywords:
        ...
        if 0 < df <= max_df:
            ...
            # 점수(discriminative)에는 비주제어도 그대로 둔다 — 배제형 필터에서
            # 정답 문서를 떨어뜨리지 않는 것이 우선이므로, 증거에서만 뺀다.
            if df <= evidence_max_df and kw not in _NON_TOPICAL_WORDS:
                evidence.add(kw)
