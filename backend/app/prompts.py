...
- ⚠️ 아래 "★ 질문 키워드와 일치하는 목차 항목" 블록은 [Step 2] 문서 선택을 위한 자료일 뿐입니다.
  "처음", "시작하면" 같은 흔한 낱말이 목차 제목에 우연히 겹쳐도 그것만으로 기술 질문이 되지는
  않으니, 이 블록을 [Step 1] 판별의 근거로 삼지 마세요. 질문 자체가 장비·현상·수치에 대해
  묻고 있는지만 보고 판단하세요 (예: "이거 처음 써보는데 뭐부터 시작하면 될까요?" → general)
...
[Step 3-1] unknown_target 판정: 질문이 **특정 제조사·브랜드·장비명을 지목**하는데 그것이
아래 문서 목록의 어떤 제조사·모델과도 대응되지 않으면 true, 그 외에는 모두 false로 설정하세요.
- 질문에 제조사·브랜드가 아예 언급되지 않았으면 false입니다 (예: "서보 알람 2051 원인" → false)
- 지목한 브랜드가 목록에 있으면 false입니다 (표기 차이·한글/영문 표기는 같은 것으로 봅니다)
- 목록에 없는 브랜드를 지목했다면, 그 뒤에 일반 장비명이 붙어 있어도 true입니다
  (예: 목록에 SETech 너트런너만 있는데 질문이 "다이치 너트런너 매뉴얼" → 다이치는 목록에 없으므로 true)
...
- 질문이 문서 목록에 없는 제조사·장비를 가리키면(unknown_target=true) suggested_questions는 빈 배열 []로 두세요 (Step 4의 예외 참고). 억지로 3개를 채우지 마세요
- unknown_target은 classification이 "general"이어도 반드시 포함하세요 (그 경우 false)
