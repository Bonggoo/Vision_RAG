...
**채점 신뢰도가 축마다 다릅니다:**
- `routing` — 신뢰도 높음. 문항의 **15%는 매뉴얼과 무관한 일상대화**(기대 `general`)로
  채워집니다. 2026-08-14 이전에는 이 대조군이 없어 기대값이 전부 `technical`이었고,
  그래서 24회 480건 연속 100%가 나왔습니다 — 그 구간의 만점은 품질 근거가 아니라
  "요청이 성공했다"는 뜻일 뿐이니 과거 회차와 비교할 때 감안하세요.
- `recall` — 신뢰도 높음. 소스 문서가 최종 참조에 잡혔거나 최소한 되묻기 후보로
  제시됐으면 통과. **`document`와 묶어 읽어 실패 계층을 가릅니다** —
  `document ✗ + recall ✓`는 선택 LLM(precision) 문제, `document ✗ + recall ✗`는
  검색·인덱싱(recall) 문제로 고칠 지점이 완전히 다릅니다.
...
**문체 혼합:** 기술문항의 30%는 **전보식(terse)** — 급한 현장 기술자가 검색창에
...
(2026-07~08 누적 실측: terse 실패율 2.3% vs sentence 0.3% — 약 7배)

**난이도 밴드:** 생성 질문의 약 절반은 본문에 모델코드(`CV-X300F`, `QD77MS` 등)가
그대로 들어가는데, 그런 질문은 문서선택이 사실상 문자열 매칭으로 풀려 쉽습니다.
결과의 `band` 필드(`coded`/`uncoded`)로 구분되며 리포트에 분리 집계됩니다.
**실질 검색 성능은 `uncoded` 쪽 숫자**를 보세요.

**생성 질문 검증:** 합성 직후 "이 모델/본 제품" 같은 지시어 질문(어떤 검색으로도
정답 문서에 도달할 수 없어 가짜 실패가 됨), 과도한 길이, 빈 문자열을 걸러내고
최대 3회까지 재생성합니다.
...
| `recall` | 소스 문서가 최종 참조에 있거나 되묻기 후보로 제시됨 (`--generate` 전용) |
